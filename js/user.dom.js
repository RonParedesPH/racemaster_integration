...
class UserDom {
    ...
    _initForm() {
        ...
        .then((module) => {
            console.log('toasts import at userListeners - OK');
            moduleToast = module.default;
            //this._toast = new module.default()

        })
        .catch((err) => {u
            console.log(`toasts import at userListeners - ${err.message}`);
        });

         const userDomFlag = document.querySelector('.userDomInjectDone');
        if (userDomFlag ? 0 : 1) {
             this._init();
        }
    ...
    _init() {
        ...
        if (this._user.token ? 1 : 0) {
            ...
            */
            ...
            if (profileElement.length > 0) {
                ...
                nameElement.innerHTML = this._user.fullName();
                if (this._user.profile.profilepic ? 1 : 0) {
                    const profilePic = document.querySelector('.user>img.profile');
                    profilePic.setAttribute('src', `img/profile/${this._user.profile.profilepic}`)
                }
            }


            let them = document.querySelectorAll(".card .h-100");
            if (them.length) {
                them[0].innerHTML =
                    "Welcome " +
                    this._user.nickName().bold() +
                    ".<br/>" +
                    (this._user.rolesList.length
                        ? "The system recognizes you to have been granted the role(s) of : " +
                        this._user.roles().bold()
            ...
            if (them ? 1 : 0) {
                ...
                let templat = '';
                let items = [];
                this._user.routeList.forEach((r) => {
                    let item = `item${i}`
                    if (headr !== r.Name.split('/')[0]) {
                        ...
                        <a class="" href="#${item}" data-href="${item}.html" data-bs-toggle="collapse" data-role="button"
                            aria-expanded="false">
                            <svg xmlns="http://www.w3.org/2000/svg" width="18" height="18" viewBox="0 0 20 20" fill="none" stroke="currentColor" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round" class="acorn-icons acorn-icons-menu-dropdown icon text-secondary">
                    ...
                    let tmp = templat.split('^')[0] +
                        `<li>
                            ...
                            <svg xmlns="http://www.w3.org/2000/svg" width="18" height="18" viewBox="0 0 20 20" fill="none" stroke="currentColor" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round" class="acorn-icons acorn-icons-accordion icon text-secondary">
                ...
                console.log('user menu at userDom - Ok')
        ...
        if (root ? 1 : 0) {
            root.classList.add('userDomApplied');
        }

    }
}

new UserDom(); // trigger initialization
