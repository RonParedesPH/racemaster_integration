...
const moduleNames = ['./helpers/messages.js',
    './components/toasts.js',
    './api/sales.api.js',
    './api/items.api.js',
    './api/outlets.api.js',
    './api/direct.api.js',
    './modules/sales.order.form.js',
    './api/sales.api.js',
];


    const loadingComplete = function(modules) { 
        console.log(modules)
        //messages = modules[1];
        Messages = modules[0];
        ...
        ItemsApi = modules[3];
        OutletsApi = modules[4];
        DirectApi = modules[5];
        SalesOrderForm = modules[6];
    ...
    window.addEventListener('load', ()=>{
        teta = modulepool.preload(moduleNames, (m)=> loadingComplete(m))
        console.log(teta)
    }); //W3C
...
class RowsAjax {
    ...
    _initObjectsAndVariables() {
        ...
        //     });
            ...
            // .then((module) => {
            //     console.log('SalesOrderForm import at dataadmin.item - OK');
            //     SalesOrderForm = module.default;
            // })
            // .catch((err) => {
            //     console.log(`SalesOrderForm import at dataadmin.item - ${err.message}`);
            // });
    }
    ...
    _onEditRowClick(rowToEdit) {
        ...
        const modal = new SalesOrderForm()
        modal.showEditor( document.getElementById('editorModal'), rowToEdit.data(), 'Edit', 
            (d) => {
                if (!callbackFlag) {
                    ...
                    d.username = _gxUser.profile.username
